...
class sx126x:
    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            ...
            print("receive message from node address with frequence %d,%d.125MHz" % (
...
node = sx126x(serial_num="/dev/ttyUSB1", freq=868, addr=20, power=22, rssi=True)


def send_deal():
    get_rec = ""
    print(
        "input a string such as  0,868,Hello World,it will send `Hello World` to lora node device of address 20 with 868MHz ")
